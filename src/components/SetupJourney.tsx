...
export function SetupJourney({
  ...
  // Add Validate Setup task - always visible
  const validateTask = {
  ...
  // Always include validate task in the list
  const allMilestones = [...milestones, validateTask];
  ...
  const recheckTasks = allMilestones.filter(m => tasksNeedingRecheck.includes(m.id));
  const activeTasks = allMilestones.filter(m => 
    !tasksNeedingRecheck.includes(m.id) && 
    !completedMilestones.includes(m.id)
  );
  const completedTasks = allMilestones.filter(m => 
    !tasksNeedingRecheck.includes(m.id) && 
    completedMilestones.includes(m.id)
  );
  ...
  // Validate Setup should only be next if all regular tasks are complete
  const nextTaskId = tasksNeedingRecheck.length > 0 
    ? tasksNeedingRecheck[0] // First task needing recheck
    : allMilestones.find(m => {
        if (m.id === 'validate-setup') {
          return allRegularCompleted && !completedMilestones.includes(m.id);
        }
        return m.required && !completedMilestones.includes(m.id);
      })?.id;
  const handleMilestoneClick = (milestoneId: MilestoneId) => {
    ...
    const isValidateTask = milestoneId === 'validate-setup';
    ...
    // For Validate Setup task, only allow if all regular tasks are complete
    if (isValidateTask && !allRegularCompleted) {
      setNudgeMessage('Please complete all other tasks first');
      setShowNudge(true);
  ...
  const renderTaskCard = (milestone: typeof allMilestones[0], index: number, isInRecheckSection = false) => {
    ...
    const isValidateTask = milestone.id === 'validate-setup';
    const isDisabled = isValidateTask && !allRegularCompleted && !isCompleted;
    const shouldScrollTo = (isInRecheckSection && needsRecheck) || (!isInRecheckSection && isNextTask);

    return (
      <motion.div 
        key={milestone.id} 
        ref={shouldScrollTo ? nextTaskRef : null} 
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-xs sm:text-sm truncate ${
                ...
                {needsRecheck ? 'Incomplete - Requires Recheck' : isDisabled ? 'Complete all other tasks first' : milestone.subtitle}
              </p>
            </div>
  ...
  // Priority: recheck tasks first, then next active task
  useEffect(() => {
    if (nextTaskRef.current && nextTaskId) {
      ...
      }, 500); // Slightly longer delay to account for section rendering
    }
  }, [completedMilestones, nextTaskId, tasksNeedingRecheck]);
  return <div className="max-w-2xl mx-auto pb-20">
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-gray-600">
          You're almost there! Complete these tasks to finish Phase 1 and unlock your Nielsen experience! 🚀
        </p>
      ...
      {/* Recheck Tasks Section */}
      {recheckTasks.length > 0 && (
        <div className="mb-8 sm:mb-10">
          <div className="flex items-center gap-2 mb-4 px-1">
            <AlertCircle className="w-5 h-5 text-blue-600" />
            <h3 className="text-lg sm:text-xl font-bold text-blue-900">
              Tasks Requiring Recheck
            </h3>
          </div>
          <div className="space-y-3 sm:space-y-4">
            {recheckTasks.map((milestone, index) => 
              renderTaskCard(milestone, index, true)
            )}
          </div>
        </div>
      )}
      ...
      {activeTasks.length > 0 && (
        <div className={recheckTasks.length > 0 ? 'mb-8 sm:mb-10' : ''}>
          <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 px-1">
            Next Steps
          </h3>
          <div className="space-y-3 sm:space-y-4">
            {activeTasks.map((milestone, index) => 
              renderTaskCard(milestone, index, false)
            )}
          </div>
        </div>
      )}
      ...
      {completedTasks.length > 0 && (
        <div className="mt-8 sm:mt-10 pt-6 sm:pt-8 border-t-2 border-gray-200">
          ...
          <div className="space-y-3 sm:space-y-4">
            {completedTasks.map((milestone, index) => 
              renderTaskCard(milestone, index, false)
            )}
          </div>
        </div>
